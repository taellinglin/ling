...
pub(crate) enum Cat {
    ...
    Hyper, Tess, Rain, Grid, Halftone, Pagoda, Torii, Cog,
    // ── Procedural / fractal textures (tex_*) ──
    Fractal,
    // ── Audio ──
    Tone, Vol, Listen, Sfx, Spectrum,
    // ── Music / MIDI ──
    Music, Note,
    // ── Cryptography ──
    Hash, Cipher, Sign, Kem, Shard,
    // ── Physics ──
    Rigid, Soft, Liquid, Force, Collide,
    // ── 3D mesh & geometry ──
    Mesh, Draw3D, Draw2D,
    // ── Math ──
    Trig, MathFn, Noise,
    ...
    Net, Neural, Behavior,
    // ── UI / dialog / holograms ──
    Widget, Hud, Dialog, Holo,
    // ── Text & vector ──
    Str, Font, Vector,
    // ── Colour, shading, scene ──
    Color, Shade, Camera, Light, Fill, Present, Window,
    // ── Input ──
    Key, Mouse,
    // ── IO / system / timing ──
    Print, File, Sys, Anim,
...
impl Cat {
    ...
    pub(crate) fn domain(self) -> &'static str {
        if is_vtex(self)    { return "texture"; }
        if is_audio(self)   { return "audio"; }
        if is_crypto(self)  { return "crypto"; }
        if is_physics(self) { return "physics"; }
        if is_ai(self)      { return "ai"; }
        match self {
            ...
            Cat::Color | Cat::Shade | Cat::Camera | Cat::Light
                | Cat::Fill | Cat::Present | Cat::Window            => "scene",
...
pub(crate) fn categorize(name: &str) -> Cat {
    ...
    if name.starts_with("vtex_rings")       || name == "ลายวงซ้อน" || name == "纹环" || name == "輪模様" || name == "輪무늬" { return Cat::Rings; }
    if name.starts_with("vtex_spiral")      || name == "ลายก้นหอย" || name == "ลายเกลียว" || name == "ลายเกลียวหมุน" || name == "纹螺" || name == "螺旋模様" || name == "나선무늬" { return Cat::Spiral; }
    if name.starts_with("vtex_star")        || name == "ลายดาว" || name == "纹星" || name == "星模様" || name == "별무늬" { return Cat::Star; }
    if name.starts_with("vtex_flower")      || name == "ลายดอกไม้" || name == "ลายดอก" || name == "纹花" || name == "花模様" || name == "꽃무늬" { return Cat::Flower; }
    if name.starts_with("vtex_lotus")       || name == "ลายบัว" || name == "ลายดอกบัว" || name == "纹莲" || name == "蓮模様" || name == "연꽃무늬" { return Cat::Lotus; }
    if name.starts_with("vtex_chakra")      || name == "ลายจักร" || name == "纹轮" || name == "輪模様" || name == "바퀴무늬" { return Cat::Chakra; }
    if name.starts_with("vtex_yantra")      || name == "ลายยันต์" || name == "纹咒" || name == "護符模様" || name == "부적무늬" { return Cat::Yantra; }
    if name.starts_with("vtex_hyper")       || name == "ลายไฮเพอร์โบลิก" || name == "ลายไฮเปอร์" || name == "纹超" || name == "超次元模様" || name == "초차원무늬" { return Cat::Hyper; }
    if name.starts_with("vtex_tessellated") || name == "ลายตาข่าย" || name == "纹镶嵌" || name == "網目模様" { return Cat::Tess; }
    if name.starts_with("vtex_letter_rain") || name.starts_with("vtex_rain") || name == "ลายอักษรไหล" || name == "ลายฝน" || name == "纹雨" || name == "文字雨" || name == "비무늬" { return Cat::Rain; }
    if name.starts_with("vtex_grid")        || name == "ลายตาราง" || name == "纹格" || name == "格子模様" || name == "격자무늬" { return Cat::Grid; }
    if name.starts_with("vtex_halftone")    || name == "ลายจุด" || name == "ลายฮาล์ฟโทน" || name == "纹半调" || name == "網点模様" || name == "망점" { return Cat::Halftone; }
    if name.starts_with("vtex_pagoda")      || name == "ลายเจดีย์" || name == "เจดีย์" || name == "纹塔" || name == "탑" { return Cat::Pagoda; }
    if name.starts_with("vtex_torii")       || name == "ประตูโทริอิ" || name == "纹鸟居" || name == "鳥居" || name == "도리이" { return Cat::Torii; }
    if name.starts_with("vtex_spiked_cog")  || name == "ฟันเฟืองหนาม" || name == "纹棘轮" || name == "歯車模様" || name == "톱니바퀴" { return Cat::Cog; }
    if name.starts_with("vtex_")            { return Cat::Tess; }

    // ── Procedural / fractal textures: tex_* ──
    if name.starts_with("tex_") { return Cat::Fractal; }

    // ── Audio ──
    if name == "audio_tone" || name == "เสียงโทน" || name == "音調" || name == "音调" || name == "음조" { return Cat::Tone; }
    if name == "audio_volume" || name == "audio_bgm_volume" || name == "ระดับเสียง" || name == "音量" || name == "음량" { return Cat::Vol; }
    if name == "audio_listener" || name == "音声リスナー" || name == "오디오리스너" || name == "音频监听" { return Cat::Listen; }
    if name.starts_with("audio_") { return Cat::Sfx; }
    if name.starts_with("mic_") || name.starts_with("fft_") { return Cat::Spectrum; }

    // ── Music / MIDI ──
    if name == "music_note_on" || name == "music_note_off" || name == "music_note" || name == "music_note_name" { return Cat::Note; }
    if name.starts_with("music_") || name.starts_with("midi_") || name.starts_with("MIDI") { return Cat::Music; }

    // ── Cryptography ──
    if name == "crypto_hash" || name == "sha3_512" || name == "blake3" || name == "hash_int" || name == "hash_str" { return Cat::Hash; }
    if name == "crypto_seal" || name == "crypto_open" || name == "encrypt" || name == "aes_gcm_256" { return Cat::Cipher; }
    if name == "ed25519" || name == "schnorr_verify" || name == "vrf_verify" || name == "derive" || name == "argon2id" { return Cat::Sign; }
    if name == "mlkem768" || name.starts_with("hybrid_") || name.starts_with("knot_") { return Cat::Kem; }
    if name == "shamir_reconstruct" { return Cat::Shard; }

    // ── Physics ──
    if name.starts_with("rb_") || name == "rigidbody" { return Cat::Rigid; }
    if name.starts_with("soft_") { return Cat::Soft; }
    if name.starts_with("liquid_") { return Cat::Liquid; }
    if matches!(name, "force"|"torque"|"spring"|"friction"|"elasticity"|"acceleration"|"apply_impulse"|"gyro") { return Cat::Force; }
    if matches!(name, "collision"|"raycast"|"aabb"|"AABB"|"constraint") { return Cat::Collide; }

    // ── 3D mesh & primitives ──
    if matches!(name, "cube"|"box"|"capsule"|"capsule_chain"|"cylinder"|"pyramid"|"icosphere"|"icosahedron"|"octahedron"|"orb_shell"|"stairs"|"frustum") { return Cat::Mesh; }
    if matches!(name, "draw_line_3d"|"draw_triangle_3d"|"line3d"|"triangle3d"|"project_3d"|"render_3d"|"flush_3d"|"font_text_3d") { return Cat::Draw3D; }

    // ── Neural / behavior / dialog (AI) ──
    if name.starts_with("nn_") { return Cat::Neural; }
    if name.starts_with("bt_") { return Cat::Behavior; }
    if name.starts_with("dialog_") { return Cat::Dialog; }

    // ── Networking ──
    if name.starts_with("net_") { return Cat::Net; }

    // ── UI widgets / HUD ──
    if matches!(name, "ui_radar"|"ui_radar3d"|"ui_minimap"|"ui_compass"|"ui_healthbar"|"ui_reticle"|"ui_target"|"ui_gauge"|"ui_gauge3d"|"ui_vu"|"ui_battery"|"ui_segbar"|"ui_bar"|"ui_progress"|"ui_cooldown"|"ui_scanlines"|"ui_vignette"|"ui_spark"|"ui_ring"|"ui_counter") { return Cat::Hud; }
    if name.starts_with("ui_") { return Cat::Widget; }

    // ── Holograms / particles ──
    if matches!(name, "holo_points"|"holo_fragment_count"|"knot_points"|"sparkle"|"neon"|"psychedelic") { return Cat::Holo; }

    // ── Fonts & vector text ──
    if name.starts_with("font_") { return Cat::Font; }
    if name.starts_with("svg_") { return Cat::Vector; }

    // ── Strings ──
    if name.starts_with("str_") || matches!(name, "split"|"join"|"trim"|"substr"|"format"|"to_str"|"num_str"|"starts_with"|"ends_with") { return Cat::Str; }

    // ── Math: trig / noise / general ──
    if matches!(name, "sin"|"cos"|"tan"|"asin"|"acos"|"atan"|"atan2"|"arcsin"|"arccos"|"arctan"|"arctan2"|"tanh"|"tanhf"|"hypot") { return Cat::Trig; }
    if matches!(name, "perlin"|"perlin3"|"noise2"|"fbm"|"vnoise"|"smoothstep") { return Cat::Noise; }
    if matches!(name, "sqrt"|"cbrt"|"pow"|"exp"|"ln"|"log"|"log2"|"log10"|"abs"|"floor"|"ceil"|"round"|"trunc"|"fract"|"sign"|"clamp"|"lerp"|"min"|"max"|"rand"|"pi"|"tau") { return Cat::MathFn; }

    // ── Colour & shading ──
    if matches!(name, "hsl_color"|"hsv_to_rgb"|"set_color"|"set_color_hsl"|"color"|"lerp_color"|"gfx_color") { return Cat::Color; }
    if matches!(name, "set_shade_mode"|"set_rim"|"set_fog"|"set_ambient"|"set_cel_bands"|"set_blend"|"set_shadow_color"|"set_projection"|"set_zdist") { return Cat::Shade; }

    // ── Camera / lights ──
    if matches!(name, "set_camera"|"set_camera_pos"|"move_camera") { return Cat::Camera; }
    if matches!(name, "add_light"|"clear_lights") { return Cat::Light; }

    // ── Window / present / fill ──
    if matches!(name, "open_window"|"open_fullscreen"|"gfx_window"|"fullscreen"|"windowed"|"wait_window"|"is_open"|"window_is_open"|"gfx_is_open"|"gfx_wait"|"resolution"|"get_width"|"get_height") { return Cat::Window; }
    if matches!(name, "present"|"แสดงผล"|"gfx_present"|"render"|"render_3d"|"flush_3d") { return Cat::Present; }
    if matches!(name, "fill"|"เติม"|"gfx_fill"|"clear") { return Cat::Fill; }

    // ── 2D drawing ──
    if name.starts_with("draw_") || matches!(name, "gfx_line"|"gfx_pixel"|"gfx_triangle"|"line"|"triangle"|"pixel") { return Cat::Draw2D; }

    // ── Input ──
    if name.starts_with("mouse_") || matches!(name, "capture_mouse"|"release_mouse") { return Cat::Mouse; }
    if name.starts_with("key_") || matches!(name, "keys"|"text_poll"|"text_get") { return Cat::Key; }

    // ── IO / system ──
    if matches!(name, "print"|"println"|"print_file"|"imprimir"|"afficher"|"вывести"|"พิมพ์"|"打印") { return Cat::Print; }
    if matches!(name, "read_file"|"write_file"|"อ่านไฟล์"|"เขียนไฟล์") { return Cat::File; }
    if matches!(name, "system"|"get_args"|"time_now"|"sleep"|"sleep_ms") { return Cat::Sys; }

    // ── Animation / timing (Anima organic drivers + classic timing) ──
    if matches!(name, "animation"|"ease"|"tick"|"delta_time"|"frame_count"|"frame"|"record_frame"|"record_count"
        |"tween"|"tween_ease"|"breathe"|"wobble"|"gait_phase"|"gait_swing"|"gait_lift"|"spring_to"|"ik2") { return Cat::Anim; }
    // ── Anima mechanical drivers — reuse the gear (Cog) glyph ──
    if matches!(name, "gear_couple"|"gear_train"|"cam_lift"|"piston"|"rack") { return Cat::Cog; }
...
pub(crate) fn is_vtex(c: Cat) -> bool {
    matches!(c, Cat::Rings|Cat::Spiral|Cat::Star|Cat::Flower|Cat::Lotus|
               ...
               Cat::Grid|Cat::Halftone|Cat::Pagoda|Cat::Torii|Cat::Cog)
}
pub(crate) fn is_audio(c: Cat) -> bool {
    matches!(c, Cat::Tone|Cat::Vol|Cat::Listen|Cat::Sfx|Cat::Spectrum)
}
pub(crate) fn is_crypto(c: Cat) -> bool {
    matches!(c, Cat::Hash|Cat::Cipher|Cat::Sign|Cat::Kem|Cat::Shard)
}
pub(crate) fn is_physics(c: Cat) -> bool {
    matches!(c, Cat::Rigid|Cat::Soft|Cat::Liquid|Cat::Force|Cat::Collide)
}
    ...
    match cat {
        ...
        Cat::Torii => {
            format!(
                r##"<line x1="{}" y1="{}" x2="{}" y2="{}" stroke="{c}" stroke-width="1.6" opacity="0.9"/>
                   ...
                   <line x1="{}" y1="{}" x2="{}" y2="{}" stroke="{c}" stroke-width="1.5" opacity="0.9"/>"##,
                p(cx - r*0.95), p(cy - r*0.6), p(cx + r*0.95), p(cy - r*0.6),
                p(cx - r*0.75), p(cy - r*0.2), p(cx + r*0.75), p(cy - r*0.2),
                p(cx - r*0.55), p(cy - r*0.6), p(cx - r*0.55), p(cy + r*0.85),
                p(cx + r*0.55), p(cy - r*0.6), p(cx + r*0.55), p(cy + r*0.85)
            )
        }
        Cat::Cog => {
            ...
            write!(s, r##"<circle cx="{}" cy="{}" r="{}" fill="{c}" opacity="0.55"/>
                         <circle cx="{}" cy="{}" r="{}" fill="#0b0b1a" opacity="0.9"/>"##,
                p(cx), p(cy), p(r*0.7), p(cx), p(cy), p(r*0.3)).ok();
            s
        }
        Cat::Fractal => {
            let outer = format!("{},{} {},{} {},{}",
                p(cx), p(cy - r*0.9), p(cx + r*0.85), p(cy + r*0.6), p(cx - r*0.85), p(cy + r*0.6));
            let inner = format!("{},{} {},{} {},{}",
                p(cx), p(cy + r*0.6), p(cx - r*0.42), p(cy - r*0.12), p(cx + r*0.42), p(cy - r*0.12));
            format!(
        ...
        Cat::Sfx => {
            format!(
                ...
                p(cx + r*0.2), p(cy - r*0.9),
                p(cx - r*0.5), p(cy + r*0.1),
                p(cx - r*0.05), p(cy + r*0.1),
                p(cx - r*0.2), p(cy + r*0.9),
                p(cx + r*0.5), p(cy - r*0.1),
                p(cx + r*0.05), p(cy - r*0.1)
            )
        ...
        Cat::Music => {
            format!(
                ...
                p(cx - r*0.35), p(cy + r*0.55), p(r*0.32), p(r*0.24), p(cx - r*0.35), p(cy + r*0.55),
                p(cx - r*0.05), p(cy + r*0.5), p(cx - r*0.05), p(cy - r*0.8),
                p(cx - r*0.05), p(cy - r*0.8), p(cx + r*0.5), p(cy - r*0.65), p(cx + r*0.55), p(cy - r*0.2), p(cx + r*0.3), p(cy - r*0.05)
            )
        }
        Cat::Note => {
            format!(
                ...
                p(cx - r*0.55), p(cy + r*0.55), p(r*0.24),
                p(cx + r*0.55), p(cy + r*0.3), p(r*0.24),
                p(cx - r*0.33), p(cy + r*0.55), p(cx - r*0.33), p(cy - r*0.6),
                p(cx + r*0.77), p(cy + r*0.3), p(cx + r*0.77), p(cy - r*0.85),
                p(cx - r*0.33), p(cy - r*0.6), p(cx + r*0.77), p(cy - r*0.85)
            )
        }
        // ── Cryptography ──────────────────────────────────────────────────────
        Cat::Hash => {
            ...
            write!(s, r##"<circle cx="{}" cy="{}" r="{}" fill="{c}" opacity="0.8"/>"##, p(cx), p(cy), p(r*0.12)).ok();
            s
        }
        Cat::Cipher => {
            format!(
                r##"<path d="M {},{} A {},{} 0 0,1 {},{}" fill="none" stroke="{c}" stroke-width="1.4" opacity="0.85"/>
                   ...
                   <circle cx="{}" cy="{}" r="{}" fill="#0b0b1a" opacity="0.9"/>"##,
                p(cx - r*0.45), p(cy - r*0.05), p(r*0.45), p(r*0.45), p(cx + r*0.45), p(cy - r*0.05),
                p(cx - r*0.7), p(cy - r*0.05), p(r*1.4), p(r*0.95),
                p(cx), p(cy + r*0.4), p(r*0.14)
            )
        }
        Cat::Sign => {
            format!(
                ...
                p(cx - r*0.9), p(cy + r*0.4),
                p(cx - r*0.4), p(cy - r*0.7), p(cx), p(cy + r*0.6), p(cx + r*0.3), p(cy - r*0.1),
                p(cx + r*0.7), p(cy - r*0.7), p(cx + r*0.9), p(cy + r*0.3),
                p(cx - r), p(cy + r*0.75), p(cx + r), p(cy + r*0.75)
            )
        }
        Cat::Kem => {
            format!(
                r##"<circle cx="{}" cy="{}" r="{}" fill="none" stroke="{c}" stroke-width="1.4" opacity="0.9"/>
                   ...
                   <line x1="{}" y1="{}" x2="{}" y2="{}" stroke="{c}" stroke-width="1.3" opacity="0.85"/>
                   <line x1="{}" y1="{}" x2="{}" y2="{}" stroke="{c}" stroke-width="1.3" opacity="0.85"/>"##,
                p(cx - r*0.5), p(cy - r*0.4), p(r*0.4),
                p(cx - r*0.18), p(cy - r*0.12), p(cx + r*0.8), p(cy + r*0.75),
                p(cx + r*0.55), p(cy + r*0.5), p(cx + r*0.8), p(cy + r*0.25),
                p(cx + r*0.8), p(cy + r*0.75), p(cx + r*1.05), p(cy + r*0.5)
            )
        }
        Cat::Shard => {
            format!(
                ...
                p(cx), p(cy - r), p(cx - r*0.85), p(cy + r*0.5), p(cx), p(cy + r*0.2),
                p(cx), p(cy - r), p(cx + r*0.85), p(cy + r*0.5), p(cx), p(cy + r*0.2),
                p(cx), p(cy - r), p(cx + r*0.85), p(cy + r*0.5), p(cx - r*0.85), p(cy + r*0.5)
            )
        }
        // ── Physics ───────────────────────────────────────────────────────────
        Cat::Rigid => {
            format!(
                ...
                p(cx), p(cy - r*0.9), p(cx + r*0.85), p(cy - r*0.4), p(cx), p(cy + r*0.1), p(cx - r*0.85), p(cy - r*0.4),
                p(cx - r*0.85), p(cy - r*0.4), p(cx), p(cy + r*0.1), p(cx), p(cy + r*0.95), p(cx - r*0.85), p(cy + r*0.45),
                p(cx + r*0.85), p(cy - r*0.4), p(cx), p(cy + r*0.1), p(cx), p(cy + r*0.95), p(cx + r*0.85), p(cy + r*0.45)
            )
        }
        Cat::Soft => {
            format!(
                ...
                p(cx), p(cy - r*0.85),
                p(cx + r*0.7), p(cy - r*0.9), p(cx + r*0.95), p(cy - r*0.2), p(cx + r*0.8), p(cy + r*0.4),
                p(cx + r*0.6), p(cy + r*0.95), p(cx - r*0.1), p(cy + r*0.9), p(cx - r*0.6), p(cy + r*0.7),
                p(cx - r*0.95), p(cy + r*0.5), p(cx - r*0.9), p(cy - r*0.3), p(cx - r*0.7), p(cy - r*0.6),
                p(cx - r*0.5), p(cy - r*0.85), p(cx - r*0.2), p(cy - r*0.95), p(cx), p(cy - r*0.85)
            )
        }
        Cat::Liquid => {
            format!(
                ...
                p(cx), p(cy - r*0.9),
                p(cx + r*0.75), p(cy - r*0.1), p(cx + r*0.6), p(cy + r*0.8), p(cx), p(cy + r*0.85),
                p(cx - r*0.6), p(cy + r*0.8), p(cx - r*0.75), p(cy - r*0.1), p(cx), p(cy - r*0.9),
                p(cx - r*0.25), p(cy + r*0.25), p(r*0.16), p(r*0.28)
            )
        }
        Cat::Force => {
            format!(
                r##"<line x1="{}" y1="{}" x2="{}" y2="{}" stroke="{c}" stroke-width="1.8" opacity="0.9" stroke-linecap="round"/>
                   <polygon points="{},{} {},{} {},{}" fill="{c}" opacity="0.9"/>"##,
                p(cx - r*0.7), p(cy + r*0.7), p(cx + r*0.45), p(cy - r*0.45),
                p(cx + r*0.8), p(cy - r*0.8), p(cx + r*0.2), p(cy - r*0.7), p(cx + r*0.7), p(cy - r*0.15)
            )
        }
        Cat::Collide => {
            let mut s = String::new();
            for i in 0..8 {
                let a = (i as f32 * 45.0).to_radians();
                let r1 = r * 0.3; let r2 = if i % 2 == 0 { r * 0.95 } else { r * 0.6 };
            ...
            write!(s, r##"<circle cx="{}" cy="{}" r="{}" fill="{c}" opacity="0.8"/>"##, p(cx), p(cy), p(r*0.18)).ok();
            s
        }
        // ── 3D mesh & drawing ─────────────────────────────────────────────────
        Cat::Mesh => {
            format!(
                ...
                p(cx), p(cy), p(r*0.9),
                p(cx), p(cy), p(r*0.38), p(r*0.9),
                p(cx), p(cy), p(r*0.9), p(r*0.38),
                p(cx - r*0.9), p(cy), p(cx + r*0.9), p(cy)
            )
        }
        Cat::Draw3D => {
            format!(
                r##"<line x1="{}" y1="{}" x2="{}" y2="{}" stroke="{c}" stroke-width="1.4" opacity="0.9"/>
                   ...
                   <circle cx="{}" cy="{}" r="{}" fill="{c}" opacity="0.9"/>"##,
                p(cx), p(cy + r*0.3), p(cx), p(cy - r*0.9),
                p(cx), p(cy + r*0.3), p(cx + r*0.9), p(cy + r*0.7),
                p(cx), p(cy + r*0.3), p(cx - r*0.85), p(cy + r*0.6),
                p(cx), p(cy + r*0.3), p(r*0.13)
            )
        }
        Cat::Draw2D => {
            format!(
                r##"<line x1="{}" y1="{}" x2="{}" y2="{}" stroke="{c}" stroke-width="1.6" opacity="0.9" stroke-linecap="round"/>
                   <circle cx="{}" cy="{}" r="{}" fill="{c}" opacity="0.9"/>
                   <circle cx="{}" cy="{}" r="{}" fill="{c}" opacity="0.9"/>"##,
                p(cx - r*0.8), p(cy + r*0.8), p(cx + r*0.8), p(cy - r*0.8),
                p(cx - r*0.8), p(cy + r*0.8), p(r*0.16),
                p(cx + r*0.8), p(cy - r*0.8), p(r*0.16)
            )
        }
        // ── Math ──────────────────────────────────────────────────────────────
        Cat::Trig => {
            format!(
                ...
                p(cx - r*0.8), p(cy + r*0.7), p(cx + r*0.8), p(cy + r*0.7), p(cx - r*0.8), p(cy - r*0.7),
                p(cx - r*0.35), p(cy + r*0.7), p(r*0.45), p(r*0.45), p(cx - r*0.8), p(cy + r*0.38)
            )
        }
        Cat::MathFn => {
            format!(
                ...
                p(cx - r*0.9), p(cy), p(cx + r*0.9), p(cy),
                p(cx), p(cy - r*0.9), p(cx), p(cy + r*0.9),
                p(cx - r*0.8), p(cy - r*0.7), p(cx), p(cy + r*1.1), p(cx + r*0.8), p(cy - r*0.7)
            )
        }
        Cat::Noise => {
            format!(
                r##"<polyline points="{},{} {},{} {},{} {},{} {},{} {},{} {},{} {},{} {},{}" fill="none" stroke="{c}" stroke-width="1.3" opacity="0.85" stroke-linejoin="round"/>"##,
                p(cx - r*0.9), p(cy + r*0.1),
                p(cx - r*0.65), p(cy - r*0.6),
                p(cx - r*0.4), p(cy + r*0.5),
                p(cx - r*0.15), p(cy - r*0.4),
                p(cx + r*0.1), p(cy + r*0.7),
                p(cx + r*0.35), p(cy - r*0.5),
                p(cx + r*0.6), p(cy + r*0.3),
                p(cx + r*0.8), p(cy - r*0.3),
                p(cx + r*0.95), p(cy + r*0.2)
            )
        }
        // ── Networking & AI ───────────────────────────────────────────────────
        Cat::Net => {
            ...
            write!(s, r##"<circle cx="{}" cy="{}" r="{}" fill="{c}" opacity="0.9"/>"##, p(cx), p(cy), p(r*0.2)).ok();
            for &(nx, ny) in nodes.iter() {
                write!(s, r##"<circle cx="{}" cy="{}" r="{}" fill="{c}" opacity="0.8"/>"##, p(cx + nx*r), p(cy + ny*r), p(r*0.18)).ok();
            }
            s
        }
        Cat::Neural => {
            ...
            let l1 = [-0.3f32, 0.3];
            let x0 = cx - r*0.7; let x1 = cx; let x2 = cx + r*0.7;
            for &a in l0.iter() { for &b in l1.iter() {
            ...
            for &a in l0.iter() { write!(s, r##"<circle cx="{}" cy="{}" r="{}" fill="{c}" opacity="0.85"/>"##, p(x0), p(cy + a*r), p(r*0.14)).ok(); }
            for &b in l1.iter() { write!(s, r##"<circle cx="{}" cy="{}" r="{}" fill="{c}" opacity="0.85"/>"##, p(x1), p(cy + b*r), p(r*0.14)).ok(); }
            write!(s, r##"<circle cx="{}" cy="{}" r="{}" fill="{c}" opacity="0.85"/>"##, p(x2), p(cy), p(r*0.14)).ok();
            s
        }
        Cat::Behavior => {
            format!(
                r##"<line x1="{}" y1="{}" x2="{}" y2="{}" stroke="{c}" stroke-width="1.0" opacity="0.7"/>
                   ...
                   <circle cx="{}" cy="{}" r="{}" fill="{c}" opacity="0.8"/>
                   <circle cx="{}" cy="{}" r="{}" fill="{c}" opacity="0.8"/>"##,
                p(cx), p(cy - r*0.6), p(cx - r*0.7), p(cy + r*0.6),
                p(cx), p(cy - r*0.6), p(cx + r*0.7), p(cy + r*0.6),
                p(cx), p(cy - r*0.6), p(r*0.2),
                p(cx - r*0.7), p(cy + r*0.6), p(r*0.18),
                p(cx + r*0.7), p(cy + r*0.6), p(r*0.18)
            )
        }
        // ── UI / dialog / holograms ───────────────────────────────────────────
        Cat::Widget => {
            format!(
                ...
                p(cx - r*0.85), p(cy - r*0.75), p(r*1.7), p(r*1.5),
                p(cx - r*0.85), p(cy - r*0.3), p(cx + r*0.85), p(cy - r*0.3),
                p(cx - r*0.5), p(cy + r*0.1), p(r*1.0), p(r*0.45)
            )
        }
        Cat::Hud => {
            format!(
                ...
                p(cx - r*0.9), p(cy), p(cx + r*0.9), p(cy),
                p(cx), p(cy - r*0.9), p(cx), p(cy + r*0.9),
                p(cx), p(cy), p(cx + r*0.64), p(cy - r*0.64),
                p(cx + r*0.4), p(cy - r*0.4), p(r*0.13)
            )
        }
        Cat::Dialog => {
            format!(
                r##"<rect x="{}" y="{}" width="{}" height="{}" rx="4" fill="{c}" opacity="0.4" stroke="{c}" stroke-width="1.0"/>
                   ...
                   <circle cx="{}" cy="{}" r="{}" fill="#0b0b1a" opacity="0.85"/>
                   <circle cx="{}" cy="{}" r="{}" fill="#0b0b1a" opacity="0.85"/>"##,
                p(cx - r*0.9), p(cy - r*0.8), p(r*1.8), p(r*1.2),
                p(cx - r*0.4), p(cy + r*0.4), p(cx - r*0.1), p(cy + r*0.4), p(cx - r*0.55), p(cy + r*0.9),
                p(cx - r*0.4), p(cy - r*0.2), p(r*0.1),
                p(cx), p(cy - r*0.2), p(r*0.1),
                p(cx + r*0.4), p(cy - r*0.2), p(r*0.1)
            )
        }
        Cat::Holo => {
        ...
        Cat::Str => {
            ...
            for &dx in [-0.5f32, 0.3].iter() {
                write!(s, r##"<circle cx="{}" cy="{}" r="{}" fill="{c}" opacity="0.85"/>
                             <circle cx="{}" cy="{}" r="{}" fill="{c}" opacity="0.85"/>"##,
                    p(cx + dx*r), p(cy - r*0.4), p(r*0.16),
                    p(cx + dx*r + r*0.3), p(cy - r*0.4), p(r*0.16)).ok();
            }
            for j in 0..2 {
        ...
        Cat::Font => {
            format!(
                r##"<polyline points="{},{} {},{} {},{}" fill="none" stroke="{c}" stroke-width="1.5" opacity="0.9" stroke-linejoin="round"/>
                   <line x1="{}" y1="{}" x2="{}" y2="{}" stroke="{c}" stroke-width="1.3" opacity="0.85"/>"##,
                p(cx - r*0.7), p(cy + r*0.8), p(cx), p(cy - r*0.85), p(cx + r*0.7), p(cy + r*0.8),
                p(cx - r*0.38), p(cy + r*0.15), p(cx + r*0.38), p(cy + r*0.15)
            )
        }
        Cat::Vector => {
            format!(
                r##"<path d="M {},{} C {},{} {},{} {},{}" fill="none" stroke="{c}" stroke-width="1.4" opacity="0.9"/>
                   ...
                   <circle cx="{}" cy="{}" r="{}" fill="none" stroke="{c}" stroke-width="1.0" opacity="0.7"/>"##,
                p(cx - r*0.8), p(cy + r*0.7), p(cx - r*0.3), p(cy - r*0.9), p(cx + r*0.3), p(cy - r*0.9), p(cx + r*0.8), p(cy + r*0.7),
                p(cx - r*0.8), p(cy + r*0.7), p(cx - r*0.3), p(cy - r*0.9),
                p(cx + r*0.8), p(cy + r*0.7), p(cx + r*0.3), p(cy - r*0.9),
                p(cx - r*0.9), p(cy + r*0.6), p(r*0.2), p(r*0.2),
                p(cx + r*0.7), p(cy + r*0.6), p(r*0.2), p(r*0.2),
                p(cx - r*0.3), p(cy - r*0.9), p(r*0.14),
                p(cx + r*0.3), p(cy - r*0.9), p(r*0.14)
            )
        }
        // ── Colour / shading / scene ──────────────────────────────────────────
        Cat::Color => {
            format!(
                ...
                p(cx), p(cy - r*0.4), p(r*0.6),
                p(cx - r*0.45), p(cy + r*0.35), p(r*0.6),
                p(cx + r*0.45), p(cy + r*0.35), p(r*0.6)
            )
        }
        Cat::Shade => {
            format!(
                ...
                p(cx), p(cy), p(r*0.9),
                p(cx), p(cy - r*0.9), p(r*0.9), p(r*0.9), p(cx), p(cy + r*0.9), p(r*0.45), p(r*0.9), p(cx), p(cy - r*0.9),
                p(cx + r*0.35), p(cy - r*0.35), p(r*0.18)
            )
        }
        Cat::Window => {
            format!(
                r##"<rect x="{}" y="{}" width="{}" height="{}" rx="2" fill="none" stroke="{c}" stroke-width="1.3" opacity="0.85"/>
                   <line x1="{}" y1="{}" x2="{}" y2="{}" stroke="{c}" stroke-width="1.0" opacity="0.7"/>
                   <line x1="{}" y1="{}" x2="{}" y2="{}" stroke="{c}" stroke-width="1.0" opacity="0.7"/>"##,
                p(cx - r*0.85), p(cy - r*0.85), p(r*1.7), p(r*1.7),
                p(cx), p(cy - r*0.85), p(cx), p(cy + r*0.85),
                p(cx - r*0.85), p(cy), p(cx + r*0.85), p(cy)
            )
        }
        // ── Input ─────────────────────────────────────────────────────────────
        Cat::Key => {
            format!(
                ...
                p(cx - r*0.8), p(cy - r*0.8), p(r*1.6), p(r*1.6),
                p(cx - r*0.45), p(cy - r*0.45), p(r*0.9), p(r*0.9)
            )
        }
        Cat::Mouse => {
            format!(
                ...
                p(cx - r*0.55), p(cy - r*0.85), p(r*1.1), p(r*1.7), p(r*0.55),
                p(cx), p(cy - r*0.85), p(cx), p(cy - r*0.1),
                p(cx - r*0.08), p(cy - r*0.7), p(r*0.16), p(r*0.4)
            )
        }
        // ── IO / system / timing ──────────────────────────────────────────────
        Cat::Print => {
            let mut s = format!(
                ...
                p(cx - r*0.6), p(cy - r*0.85), p(r*1.2), p(r*1.7));
            for j in 0..4 {
        ...
        Cat::File => {
            format!(
                ...
                p(cx - r*0.6), p(cy - r*0.85),
                p(cx + r*0.25), p(cy - r*0.85),
                p(cx + r*0.6), p(cy - r*0.45),
                p(cx + r*0.6), p(cy + r*0.85),
                p(cx - r*0.6), p(cy + r*0.85),
                p(cx + r*0.25), p(cy - r*0.85), p(cx + r*0.25), p(cy - r*0.45), p(cx + r*0.6), p(cy - r*0.45)
            )
        }
        Cat::Sys => {
            format!(
                ...
                p(cx - r*0.9), p(cy - r*0.7), p(r*1.8), p(r*1.4),
                p(cx - r*0.45), p(cy - r*0.2), p(cx - r*0.1), p(cy + r*0.15), p(cx - r*0.45), p(cy + r*0.5),
                p(cx + r*0.1), p(cy + r*0.5), p(cx + r*0.5), p(cy + r*0.5)
            )
        }
        Cat::Anim => {
            let mut s = String::new();
            for i in 0..6 {
                ...
                write!(s, r##"<circle cx="{}" cy="{}" r="{}" fill="{c}" opacity="{:.2}"/>"##,
                    p(cx + rad*a.cos()), p(cy + rad*a.sin()), p(dotr), 0.35 + i as f32*0.1).ok();
            }
            s
        }
        Cat::Const => {
...
fn render_header(filename: &str, funcs: &[FuncCard]) -> String {
    ...
    let count = |pred: fn(Cat) -> bool| -> usize {
        funcs.iter().flat_map(|f| &f.calls).filter(|c| pred(c.cat)).map(|c| c.count).sum()
    };
    ...
    for (n, lbl) in [(n_vtex, "vtex"), (n_audio, "audio"), (n_crypto, "crypto"),
                     (n_physics, "physics"), (n_ai, "ai")] {
        if n > 0 { stats.push(format!("{n} {lbl}")); }
    }
...
fn render_legend(funcs: &[FuncCard]) -> String {
    ...
    for cat in present {
        ...
        if dom != cur_domain {
            ...
            if lx > x_start && lx + tag_w > x_max { row += 1; lx = x_start; }
            if row > 1 { break; } // at most two rows in the band
            let ly = row_y(row);
            s.push_str(&format!(
        ...
        if lx + item_w > x_max { row += 1; lx = x_start; }
        if row > 1 { break; }
        let ly = row_y(row);
