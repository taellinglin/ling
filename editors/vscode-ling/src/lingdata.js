...
const categories = [
  ...
  ["Surfaces & shadows", ["set_alpha", "grad_triangle", "grad_rect", "shadow_blob", "cast_shadow", "shadow_params", "depth_triangle", "depth_line"]],
  ["Compositing (2030)", ["set_color_space", "set_gradient_space", "mix_color", "set_depth_test"]],
...
const signatures = {
  ...
  set_color_space: "set_color_space(mode: number)  // 0 sRGB · 1 linear-light",
  set_gradient_space: "set_gradient_space(mode: number)  // 0 sRGB · 1 OkLab",
  mix_color: "mix_color(r0, g0, b0, r1, g1, b1, t: number)",
  liquid_new: "liquid_new(width: number, height: number) -> id",
  liquid_set_colors: "liquid_set_colors(id, water_r, water_g, water_b, oil_r, oil_g, oil_b)",
  liquid_splat: "liquid_splat(id, x: number, y: number, kind: number, amount: number, radius: number)",
  liquid_gravity: "liquid_gravity(id, gx: number, gy: number)",
  liquid_step: "liquid_step(id, dt: number)",
  liquid_step_all: "liquid_step_all(dt: number)  // steps every liquid grid in parallel",
  liquid_rainbow: "liquid_rainbow(id, on: number)  // flowing ROYGBIV marble",
  liquid_mix: "liquid_mix(id) -> number  // 0 separated .. 1 fully mixed",
  liquid_draw: "liquid_draw(id, sx: number, sy: number, scale: number)",
  liquid_draw_surface: "liquid_draw_surface(id, kind: number, cx, cy, cz, radius: number, height: number)  // kind: 0 plane · 1 sphere · 2 cylinder · 3 cone · 4 dome",
  vtex_grid: "vtex_grid(cx, cy, cz, ux, uy, uz, vx, vy, vz, cols, rows, cell_w, cell_h, fr, hue)",
  vtex_rings: "vtex_rings(cx, cy, cz, ux, uy, uz, vx, vy, vz, n_rings, n_sides, max_r, twist, fr, hue)",
  vtex_star: "vtex_star(cx, cy, cz, ux, uy, uz, vx, vy, vz, n_pts, r_out, r_in, rot_speed, fr, hue)",
  vtex_spiral: "vtex_spiral(cx, cy, cz, ux, uy, uz, vx, vy, vz, n_turns, max_r, steps, fr, hue)",
  vtex_flower: "vtex_flower(cx, cy, cz, ux, uy, uz, vx, vy, vz, radius, n_sides, fr, hue)",
  vtex_lotus: "vtex_lotus(cx, cy, cz, ux, uy, uz, vx, vy, vz, r_inner, r_outer, n_petals, fr, hue)",
  vtex_letter_rain: "vtex_letter_rain(cx, cy, cz, ux, uy, uz, vx, vy, vz, cols, rows, col_w, row_h, speed, fr, hue)",
  vtex_hyperbolic_uv: "vtex_hyperbolic_uv(cx, cy, cz, ux, uy, uz, vx, vy, vz, max_r, n_circles, n_rays, fr, hue)",
  vtex_halftone: "vtex_halftone(cx, cy, cz, ux, uy, uz, vx, vy, vz, cols, rows, cell_w, cell_h, density, fr, hue)",
  vtex_tessellated: "vtex_tessellated(cx, cy, cz, ux, uy, uz, vx, vy, vz, cols, rows, cell, amplitude, freq, fr, hue)",
  vtex_yantra: "vtex_yantra(cx, cy, cz, ux, uy, uz, vx, vy, vz, n_layers, max_r, fr, hue)",
  set_depth_test: "set_depth_test(on: number)",
  set_alpha: "set_alpha(a: number)",
  grad_triangle: "grad_triangle(x0, y0, r, g, b, x1, y1, r, g, b, x2, y2, r, g, b)",
  grad_rect: "grad_rect(x, y, w, h, r0, g0, b0, r1, g1, b1, dir: number)",
  shadow_blob: "shadow_blob(cx, cy, rx, ry, alpha: number)",
  cast_shadow: "cast_shadow(cx: number, cy: number, height: number)",
  shadow_params: "shadow_params(base, grow, alpha, fade, soft)",
  depth_triangle: "depth_triangle(x0, y0, x1, y1, x2, y2, z: number)",
  depth_line: "depth_line(x0, y0, x1, y1, z: number)",
