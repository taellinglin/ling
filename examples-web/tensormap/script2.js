...
const GALAXY_SPIN_SPEED = 1000000/(24*7*365);

const GALAXY_COUNT = 8;
const GALAXY_ARMS = 2;
...
let galaxies = Array.from({ length: GALAXY_COUNT }, () => ({
  ...
  angle: Math.random(),
  spin: (Math.random() < 0.5 ? -1 : 1) * 0.01,
  char: randomChar(),
  colorTime: Math.random() * 1
}));
function loopOWav() {
    const audio = new Audio('x.wav');
    audio.loop = true;
    audio.play();
  }
loopOWav();
function drawGalaxyChar(x, y, char, hslColor) {
    // Ensure alpha is slightly visible even at lowest values
    const alpha = 0.2 + Math.random() * 0.8; // Flickers between 0.2 and 1.0
  
    // Convert HSL to HSLA with flickering alpha
    let hslaColor = hslColor;
    if (hslColor.startsWith('hsl(')) {
      hslaColor = hslColor.replace('hsl(', 'hsla(').replace(')', `, ${alpha})`);
    }
...
function drawGalaxies() {
    const rainbowSteps = 7;
    const rainbowHueStep = 360 / rainbowSteps;
        ...
        galaxies.forEach(galaxy => {
          ...
          for (let a = 0; a < GALAXY_ARMS; a++) {
            ...
            for (let i = 0; i < GALAXY_POINTS; i++) {
              ...
              // Cycle through ROYGBIV per letter using fixed step offset
              const hue = (galaxy.baseHue + (i % rainbowSteps) * rainbowHueStep + galaxy.colorTime * 0.5) % 360;
