...
(function () {
  ...
  var DICT = {
    ...
    "Builtins — Toon Post-FX & Volumetric Light": ["内置 — 卡通后期与体积光", "組み込み — トゥーンポストFXとボリュメトリックライト", "내장 — 툰 포스트FX & 볼류메트릭 라이트", "ในตัว — โพสต์เอฟเฟกต์ทูนและแสงโวลูเมตริก"],
    ...
    "Glossary": ["词汇表", "用語集", "용어집", "อภิธานศัพท์"],
    "Overview": ["概览", "概要", "개요", "ภาพรวม"],
    "Keywords": ["关键字", "キーワード", "키워드", "คำสงวน"],
    "Types": ["类型", "型", "타입", "ชนิดข้อมูล"],
    "Literals & operators": ["字面量与运算符", "リテラルと演算子", "리터럴 & 연산자", "ค่าคงที่และตัวดำเนินการ"],
    "Builtins by Crate": ["按 Crate 分类的内置函数", "クレート別の組み込み", "크레이트별 내장 함수", "ฟังก์ชันในตัวตามเครต"],
    "Crates & Functions (Rust API)": ["Crate 与函数 (Rust API)", "クレートと関数 (Rust API)", "크레이트 & 함수 (Rust API)", "เครตและฟังก์ชัน (Rust API)"]
  };
