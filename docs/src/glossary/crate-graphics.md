...
| `tone_soft` | `色调柔边` | `トーンソフト` | `톤소프트` | `โทนขอบนุ่ม` |
| `set_ssao` | `环境光遮蔽` | `アンビエントオクルージョン` | `앰비언트오클루전` | `ตั้งเงาสัมผัส` |
| `set_fxaa` | `屏幕抗锯齿` | `画面アンチエイリアス` | `화면안티앨리어싱` | `ลบรอยหยัก` |
| `set_bloom` | `泛光` | `ブルーム` | `블룸` | `ตั้งบลูม` |
| `light_pool` | `光池` | `ライトプール` | `빛웅덩이` | `แอ่งแสง` |
| `light_beam` | `光柱` | `ライトビーム` | `빛기둥` | `ลำแสงไฟ` |
