...
struct Contour { start: [f32; 2], segs: Vec<Seg> }

#[derive(Clone, Debug, Default)]
struct Glyph { contours: Vec<Contour>, advance: f32 }
...
impl Collector {
    fn finish_cur(&mut self) {
        if let Some(c) = self.cur.take() {
            if !c.segs.is_empty() { self.contours.push(c); }
        }
    }
}
...
impl OutlineBuilder for Collector {
    ...
    fn line_to(&mut self, x: f32, y: f32) {
        if let Some(c) = &mut self.cur { c.segs.push(Seg::Line([x, y])); }
        self.cp = [x, y];
    }
    fn quad_to(&mut self, x1: f32, y1: f32, x: f32, y: f32) {
        if let Some(c) = &mut self.cur { c.segs.push(Seg::Quad([x1, y1], [x, y])); }
        self.cp = [x, y];
    }
    fn curve_to(&mut self, x1: f32, y1: f32, x2: f32, y2: f32, x: f32, y: f32) {
        if let Some(c) = &mut self.cur { c.segs.push(Seg::Cubic([x1, y1], [x2, y2], [x, y])); }
        self.cp = [x, y];
    }
    fn close(&mut self) { self.finish_cur(); }
}
...
fn unit(d: [f32; 2]) -> Option<[f32; 2]> {
    ...
    if l < 1e-9 { None } else { Some([d[0] / l, d[1] / l]) }
}
...
fn compress_contour(start: [f32; 2], segs: &[Seg]) -> Vec<Seg> {
    ...
    for seg in segs {
        match seg {
            ...
            Seg::Quad(c, p)      => { out.push(Seg::Quad(*c, *p));      cp = *p; line_a = None; }
            Seg::Cubic(a, b, p)  => { out.push(Seg::Cubic(*a, *b, *p)); cp = *p; line_a = None; }
        }
    }
    out
}
...
fn flat_quad(p0: [f32; 2], c: [f32; 2], p1: [f32; 2], tol: f32, out: &mut Vec<[f32; 2]>) {
    ...
    let p01 = m(p0, c); let p12 = m(c, p1); let mid = m(p01, p12);
    flat_quad(p0, p01, mid, tol, out);
    flat_quad(mid, p12, p1, tol, out);
}

fn flat_cubic(p0: [f32; 2], c1: [f32; 2], c2: [f32; 2], p1: [f32; 2], tol: f32, out: &mut Vec<[f32; 2]>) {
    let dx = p1[0] - p0[0]; let dy = p1[1] - p0[1];
    ...
    let m = |a: [f32; 2], b: [f32; 2]| [(a[0] + b[0]) * 0.5, (a[1] + b[1]) * 0.5];
    let p01 = m(p0, c1); let p12 = m(c1, c2); let p23 = m(c2, p1);
    let p012 = m(p01, p12); let p123 = m(p12, p23); let mid = m(p012, p123);
...
impl VectorFont {
    ...
    fn ensure(&mut self, ch: char) {
        if self.glyphs.contains_key(&ch) { return; }
    ...
    fn extract(&self, ch: char) -> Glyph {
        ...
        let advance = face.glyph_hor_advance(gid).map(|a| a as f32 / self.upm).unwrap_or(0.5);
        ...
        let contours = col.contours.into_iter().map(|c| {
            let start = n(c.start);
            let segs: Vec<Seg> = c.segs.iter().map(|s| match s {
    ...
    pub fn glyph_outline(&mut self, ch: char, tol_em: f32) -> GlyphOutline {
        ...
        for c in &g.contours {
            ...
            for s in &c.segs {
                match s {
                    Seg::Line(p)        => { pl.push(*p); cur = *p; }
                    Seg::Quad(ctrl, p)  => { flat_quad(cur, *ctrl, *p, tol, &mut pl); cur = *p; }
                    Seg::Cubic(a, b, p) => { flat_cubic(cur, *a, *b, *p, tol, &mut pl); cur = *p; }
                }
            }
    ...
    for c in &g.contours {
        ...
        for seg in &c.segs {
            match seg {
                ...
                Seg::Quad(a, p) =>
                    s.push_str(&format!("Q {:.4} {:.4} {:.4} {:.4}\n", a[0], a[1], p[0], p[1])),
                Seg::Cubic(a, b, p) =>
                    s.push_str(&format!("C {:.4} {:.4} {:.4} {:.4} {:.4} {:.4}\n",
    ...
    for line in text.lines() {
        ...
        if let Some(rest) = line.strip_prefix('#') {
            if let Some(i) = rest.find("adv=") {
                if let Ok(v) = rest[i + 4..].split_whitespace().next().unwrap_or("").parse::<f32>() {
                    advance = v;
                }
            }
            continue;
        ...
        match op {
            "M" => {
                if let Some(c) = cur.take() { contours.push(c); }
                cur = Some(Contour { start: [*nums.first()?, *nums.get(1)?], segs: Vec::new() });
            }
            "L" => { if let Some(c) = &mut cur { c.segs.push(Seg::Line([*nums.first()?, *nums.get(1)?])); } }
            "Q" => { if let Some(c) = &mut cur {
                c.segs.push(Seg::Quad([*nums.first()?, *nums.get(1)?], [*nums.get(2)?, *nums.get(3)?])); } }
            "C" => { if let Some(c) = &mut cur {
                c.segs.push(Seg::Cubic([*nums.first()?, *nums.get(1)?], [*nums.get(2)?, *nums.get(3)?], [*nums.get(4)?, *nums.get(5)?])); } }
            "Z" => { if let Some(c) = cur.take() { contours.push(c); } }
            _ => {}
        }
    }
    if let Some(c) = cur.take() { contours.push(c); }
...
mod tests {
    ...
    fn collinear_lines_merge() {
        let segs = vec![Seg::Line([0.5, 0.0]), Seg::Line([1.0, 0.0]), Seg::Line([1.0, 1.0])];
        let out = compress_contour([0.0, 0.0], &segs);
        // the two collinear horizontal lines collapse into one
        assert_eq!(out.len(), 2);
        match out[0] { Seg::Line(p) => assert_eq!(p, [1.0, 0.0]), _ => panic!() }
    }
    ...
    fn glyph_ling_roundtrips() {
        let g = Glyph {
            ...
            contours: vec![Contour {
                start: [0.1, 0.0],
                segs: vec![Seg::Line([0.4, 0.7]), Seg::Quad([0.5, 0.8], [0.6, 0.7]), Seg::Line([0.9, 0.0])],
            }],
        };
