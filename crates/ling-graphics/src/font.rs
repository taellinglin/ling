...
impl FontAtlas {
    ...
    pub fn get_or_rasterize(&mut self, c: char, px: f32) -> GlyphInfo {
        ...
        self.glyphs
            .get(&key)
            .cloned()
            .unwrap_or_else(|| GlyphInfo::empty(px * 0.5))
    }
    ...
    fn rasterize_glyph(&mut self, c: char, px: f32) {
        ...
        if metrics.width == 0 || metrics.height == 0 {
            let key = (c as u32, px.to_bits());
            self.glyphs
                .insert(key, GlyphInfo::empty(metrics.advance_width));
            return;
        }
    ...
    for ch in text.chars() {
        ...
        if info.size.x > 0.0 && info.size.y > 0.0 {
            ...
            vertices.push(Vertex {
                ...
                normal: Vec3::Z,
                uv: Vec2::new(info.uv_max.x, info.uv_min.y),
                color,
                tangent: Vec3::X,
            });
            vertices.push(Vertex {
            ...
            vertices.push(Vertex {
                ...
                normal: Vec3::Z,
                uv: Vec2::new(info.uv_min.x, info.uv_max.y),
                color,
                tangent: Vec3::X,
            });
...
pub fn measure_text(atlas: &mut FontAtlas, text: &str, px: f32) -> f32 {
    text.chars()
        .map(|c| atlas.get_or_rasterize(c, px).advance)
        .sum()
}
