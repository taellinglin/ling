...
pub fn simple_layout(parent: LayoutNode, count: usize, style: &Style) -> Vec<LayoutNode> {
    ...
    let available = match style.direction {
        ...
        _ => parent.height - style.padding * 2.0 - style.gap * (count - 1) as f32,
    };
        ...
        .map(|i| {
            ...
            match style.direction {
                FlexDirection::Row => LayoutNode {
                    x: parent.x + offset,
                    y: parent.y + style.padding,
                    width: child_size,
                    height: parent.height - style.padding * 2.0,
                },
                _ => LayoutNode {
                    x: parent.x + style.padding,
                    y: parent.y + offset,
                    width: parent.width - style.padding * 2.0,
                    height: child_size,
                },
            }
        })
