...
pub fn from_bytes(data: &[u8]) -> Result<MidiSong, String> {
    ...
    let tick_to_sec = |tick: u64| -> f32 {
        ...
        for i in 0..tempo_map.len() {
            ...
            if seg_start >= tick { break; }
            let seg_end = tempo_map.get(i + 1).map(|e| e.0).unwrap_or(u64::MAX).min(tick);
    ...
    for track in &smf.tracks {
        ...
        let mut pending: std::collections::HashMap<(u8, u8), Vec<(u64, u8)>> = std::collections::HashMap::new();
        for ev in track {
            tick += ev.delta.as_int() as u64;
            if let TrackEventKind::Midi { channel, message } = ev.kind {
                ...
                match message {
                    MidiMessage::NoteOn { key, vel } if vel.as_int() > 0 => {
                        pending.entry((ch, key.as_int())).or_default().push((tick, vel.as_int()));
                    }
                    MidiMessage::NoteOn { key, .. } | MidiMessage::NoteOff { key, .. } => {
                        if let Some(stack) = pending.get_mut(&(ch, key.as_int())) {
                            if let Some((start, vel)) = stack.pop() {
                                ...
                                notes.push(MidiNote { time: t, dur: (end - t).max(0.0), midi: key.as_int(), vel, channel: ch });
                            }
                        }
                    }
                    _ => {}
...
mod tests {
    use super::*;
    use midly::{Header, Format, Timing, Track, TrackEvent, TrackEventKind, MidiMessage, MetaMessage, num::*};

    /// Build a tiny SMF in memory: 120 BPM, two quarter notes (C4 then E4).
    fn synth_smf() -> Vec<u8> {
        let mut track = Track::new();
        track.push(TrackEvent { delta: 0.into(), kind: TrackEventKind::Meta(MetaMessage::Tempo(500_000.into())) });
        // C4 on @0, off @480 (one quarter at 480 tpqn)
        track.push(TrackEvent { delta: 0.into(),   kind: TrackEventKind::Midi { channel: 0.into(), message: MidiMessage::NoteOn  { key: 60.into(), vel: 100.into() } } });
        track.push(TrackEvent { delta: 480.into(), kind: TrackEventKind::Midi { channel: 0.into(), message: MidiMessage::NoteOff { key: 60.into(), vel: 0.into() } } });
        // E4 on @480, off @960
        track.push(TrackEvent { delta: 0.into(),   kind: TrackEventKind::Midi { channel: 0.into(), message: MidiMessage::NoteOn  { key: 64.into(), vel: 100.into() } } });
        track.push(TrackEvent { delta: 480.into(), kind: TrackEventKind::Midi { channel: 0.into(), message: MidiMessage::NoteOff { key: 64.into(), vel: 0.into() } } });
        track.push(TrackEvent { delta: 0.into(), kind: TrackEventKind::Meta(MetaMessage::EndOfTrack) });
        let smf = Smf { header: Header::new(Format::SingleTrack, Timing::Metrical(u15::new(480))), tracks: vec![track] };
        let mut out = Vec::new();
    ...
    fn parses_two_notes_with_tempo() {
        ...
        assert!((song.notes[1].time - 0.5).abs() < 0.02, "got {}", song.notes[1].time);
