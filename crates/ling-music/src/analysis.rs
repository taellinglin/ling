...
pub fn duration(n: usize, rate: u32) -> f32 { n as f32 / rate.max(1) as f32 }
...
fn spectrogram(mono: &[f32]) -> Vec<Vec<f32>> {
    ...
    if mono.len() < FRAME { return frames; }
    let mut pos = 0;
    let mut scratch = vec![Complex { re: 0.0, im: 0.0 }; FRAME];
    while pos + FRAME <= mono.len() {
        for i in 0..FRAME { scratch[i] = Complex { re: mono[pos + i] * window[i], im: 0.0 }; }
        fft.process(&mut scratch);
        frames.push(scratch[..bins].iter().map(|c| (c.re * c.re + c.im * c.im).sqrt()).collect());
        pos += HOP;
    }
    frames
}
...
pub fn onset_envelope(mono: &[f32]) -> Vec<f32> {
    ...
    if max > 0.0 { for e in &mut env { *e /= max; } }
    env
}
...
pub fn onsets(mono: &[f32], rate: u32) -> Vec<f32> {
    ...
    if env.is_empty() { return Vec::new(); }
...
pub fn bpm(mono: &[f32], rate: u32) -> f32 {
    ...
    let max_lag = (env_rate * 60.0 / min_bpm).ceil().min((env.len() / 2) as f32) as usize;
    ...
    for lag in min_lag..=max_lag {
        let mut ac = 0.0f32;
        for i in 0..(centered.len() - lag) { ac += centered[i] * centered[i + lag]; }
        // mild preference for mid-tempo (avoids picking very fast subdivisions)
        let weight = 1.0 - 0.15 * ((lag as f32 - (min_lag + max_lag) as f32 * 0.5).abs()
...
pub fn beat_grid(mono: &[f32], rate: u32, bpm_val: f32) -> Vec<f32> {
    ...
    while t < total { beats.push((t * 1000.0).round() / 1000.0); t += period; }
    beats
}
...
pub fn chroma(mono: &[f32], rate: u32) -> [f32; 12] {
    ...
    if sum > 0.0 { for v in &mut c { *v /= sum; } }
    c
}
...
pub fn key(mono: &[f32], rate: u32) -> (usize, bool) {
    ...
    let corr = |profile: &[f32; 12], rot: usize| -> f32 {
        let mut s = 0.0;
        for i in 0..12 { s += c[i] * profile[(i + 12 - rot) % 12]; }
        s
    };
    ...
    for root in 0..12 {
        ...
        if maj > best.2 { best = (root, true, maj); }
        if min > best.2 { best = (root, false, min); }
    }
    (best.0, best.1)
}
...
pub fn key_name(mono: &[f32], rate: u32) -> String {
    ...
    format!("{} {}", pitch_class_name(root, false), if major { "major" } else { "minor" })
}
