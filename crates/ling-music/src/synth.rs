...
impl Default for Adsr {
    fn default() -> Self {
        Self { a: 0.005, d: 0.1, s: 0.8, r: 0.3 }
    }
}
...
impl Default for Filter {
    fn default() -> Self {
        Self {
            ...
            reso: 0.2,
            env: 0.0,
            adsr: Adsr { a: 0.0, d: 0.2, s: 1.0, r: 0.3 },
            on: false,
        }
    }
}
...
impl Default for Lfo {
    fn default() -> Self {
        Self { rate: 5.0, pitch: 0.0, cutoff: 0.0, amp: 0.0 }
    }
}
...
impl Env {
    ...
    fn next(&mut self, a: &Adsr, dt: f32) -> f32 {
        match self.stage {
            ...
            Stage::Release => {
                self.level -= if a.r > 1e-5 {
                    a.s.max(self.level) * (dt / a.r)
                } else {
                    1.0
                };
...
impl Voice {
    ...
    fn next(&mut self, rate: f32) -> f32 {
        ...
        if self.envs.iter().all(|e| e.done()) {
            self.active = false;
        }
        sample
    }
...
impl Synth {
    ...
    pub fn note_off_pitch(&mut self, patch: usize, midi: i32) {
        if let Some(v) = self.voices.iter_mut().rev().find(|v| {
            v.active
                && v.midi == midi
                && v.auto_off.is_none()
...
mod tests {
    ...
    fn voice_plays_and_dies() {
        ...
        let mut peak = 0.0f32;
        for _ in 0..44100 {
            peak = peak.max(syn.next_sample().abs());
        }
    ...
    fn fm_changes_timbre() {
        ...
        p.ops = vec![
            Op {
                ratio: 1.0,
                fm_target: -1,
                adsr: Adsr { a: 0.001, d: 0.5, s: 1.0, r: 0.1 },
                ..Default::default()
            },
            Op {
                ratio: 2.0,
