//! ML-DSA-65 and ML-DSA-87 (FIPS 204, ex-Dilithium) post-quantum digital
//! signatures.
//!
...
//!
//! Two parameter sets, same wrapper shape: ML-DSA-65 (NIST security
//! category 3) and ML-DSA-87 (category 5, the highest FIPS 204 defines —
//! larger keys/signatures for a larger security margin; `lingtp://`'s
//! handshake authentication uses -87). Both come from the same `ml-dsa`
...
impl MlDsa87Keypair {
    ...
    pub fn from_seed(seed: [u8; 32]) -> Self {
        let seed: Seed = seed.into();
        Self { signing_key: SigningKey::<MlDsa87>::from_seed(&seed) }
    }
    ...
    pub fn verify(pubkey: &[u8], msg: &[u8], sig: &[u8]) -> Result<(), &'static str> {
        let vk_enc =
            EncodedVerifyingKey::<MlDsa87>::try_from(pubkey).map_err(|_| "invalid pubkey")?;
        let vk = VerifyingKey::<MlDsa87>::decode(&vk_enc);

        let sig_enc =
            EncodedSignature::<MlDsa87>::try_from(sig).map_err(|_| "invalid signature")?;
        let signature = Signature::<MlDsa87>::decode(&sig_enc).ok_or("invalid signature")?;
