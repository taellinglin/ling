...
fn vnoise(x: f32, y: f32, seed: u32) -> f32 {
    ...
    let xf = smooth(x - xi as f32); let yf = smooth(y - yi as f32);
    let a = hash2(xi,   yi,   seed); let b = hash2(xi+1, yi,   seed);
    let c = hash2(xi,   yi+1, seed); let d = hash2(xi+1, yi+1, seed);
    let ab = a + (b - a) * xf; let cd = c + (d - c) * xf;
    ab + (cd - ab) * yf
}

fn fbm(x: f32, y: f32, octaves: u32, seed: u32) -> f32 {
    let mut v = 0.0f32; let mut a = 0.5f32; let mut f = 1.0f32;
    for i in 0..octaves {
...
impl Palette {
    ...
    pub fn rainbow()     -> Self { Self::new([0.5,0.5,0.5],[0.5,0.5,0.5],[1.0,1.0,1.0],[0.0,0.333,0.667]) }
    pub fn fire()        -> Self { Self::new([0.8,0.4,0.1],[0.7,0.3,0.1],[1.0,0.5,0.3],[0.0,0.5,0.8]) }
    pub fn ocean()       -> Self { Self::new([0.1,0.4,0.7],[0.3,0.3,0.4],[0.8,1.0,0.5],[0.3,0.0,0.6]) }
    pub fn psychedelic() -> Self { Self::new([0.5,0.5,0.5],[0.8,0.8,0.8],[1.0,1.3,0.7],[0.0,0.15,0.3]) }
    pub fn neon()        -> Self { Self::new([0.5,0.5,0.5],[0.5,0.5,0.5],[2.0,1.0,0.0],[0.5,0.2,0.25]) }
    pub fn forest()      -> Self { Self::new([0.3,0.5,0.2],[0.2,0.3,0.1],[1.0,0.5,0.8],[0.1,0.3,0.6]) }

    pub fn eval(&self, t: f32) -> [f32; 3] {
        [0,1,2].map(|i| {
            (self.a[i] + self.b[i] * (std::f32::consts::TAU * (self.c[i] * t + self.d[i])).cos()).clamp(0.0, 1.0)
        })
    }
...
pub struct TexBuf { pub data: Vec<u8>, pub width: u32, pub height: u32 }
impl TexBuf {
    pub fn new(width: u32, height: u32) -> Self {
        Self { data: vec![0u8; (width * height * 4) as usize], width, height }
    }
    pub fn set(&mut self, x: u32, y: u32, rgba: [u8; 4]) {
        let i = ((y * self.width + x) * 4) as usize;
        if i + 3 < self.data.len() { self.data[i..i+4].copy_from_slice(&rgba); }
    }
...
pub fn checkerboard(width: u32, height: u32, tiles: u32, color_a: [u8; 4], color_b: [u8; 4]) -> Vec<u8> {
...
pub fn noise(width: u32, height: u32, scale: f32, octaves: u32, seed: u64, palette: &Palette) -> Vec<u8> {
    let mut tex = TexBuf::new(width, height);
    for y in 0..height {
        for x in 0..width {
            let v = fbm(x as f32 * scale / width as f32,
...
pub fn mandelbrot(width: u32, height: u32, palette: &Palette,
                  zoom: f64, cx: f64, cy: f64, max_iter: u32) -> Vec<u8> {
    let mut tex = TexBuf::new(width, height);
    for py in 0..height {
        for px in 0..width {
            ...
            while i < max_iter && x*x + y*y < 4.0 {
                let tmp = x*x - y*y + zx;
                y = 2.0*x*y + zy; x = tmp; i += 1;
            }
            let t = if i == max_iter { 0.0 } else {
...
pub fn julia(width: u32, height: u32, palette: &Palette,
             c_re: f64, c_im: f64, max_iter: u32) -> Vec<u8> {
    let mut tex = TexBuf::new(width, height);
    for py in 0..height {
        for px in 0..width {
...
pub fn voronoi(width: u32, height: u32, cell_count: u32, seed: u64, palette: &Palette) -> Vec<u8> {
    ...
    let cells: Vec<[f32; 2]> = (0..cell_count).map(|i| {
    ...
    for py in 0..height {
        for px in 0..width {
            ...
            let (min_d, nearest) = cells.iter().enumerate().fold(
                (f32::MAX, 0usize),
                |(d, idx), (i, &[cx, cy])| {
...
pub fn hypnotic_spiral(width: u32, height: u32, freq: f32, bands: u32, time: f32, palette: &Palette) -> Vec<u8> {
    let mut tex = TexBuf::new(width, height);
    for y in 0..height {
        for x in 0..width {
            let nx = x as f32 / width as f32 - 0.5;
            ...
            let t = ((r * freq - theta / std::f32::consts::TAU + time * 0.5) * bands as f32 % 1.0).abs();
...
pub fn freq_map(bands: &[f32], palette: &Palette, width: u32, height: u32, time: f32, speed: f32) -> Vec<u8> {
    let mut tex = TexBuf::new(width, height);
    ...
    for y in 0..height {
        for x in 0..width {
            ...
            if y >= height - fill_y {
                ...
                tex.set(x, y, [(r * brightness * 255.0) as u8, (g * brightness * 255.0) as u8, (b * brightness * 255.0) as u8, 255]);
            }
        }
    }
    tex.into_vec()
...
pub fn halftone(width: u32, height: u32, dot_size: f32, palette: &Palette, time: f32) -> Vec<u8> {
    ...
    for y in 0..height {
        for x in 0..width {
            ...
            let a = if r < 0.7 { ((0.7 - r) / 0.7).clamp(0.0, 1.0) } else { 0.0 };
            tex.set(x, y, [(pr * 255.0) as u8, (pg * 255.0) as u8, (pb * 255.0) as u8, (a * 255.0) as u8]);
        }
    }
    tex.into_vec()
}
...
pub fn ripple(width: u32, height: u32, freq: f32, cx: f32, cy: f32, time: f32, palette: &Palette) -> Vec<u8> {
    let mut tex = TexBuf::new(width, height);
    for y in 0..height {
        for x in 0..width {
