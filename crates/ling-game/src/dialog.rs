...
impl Dialog {
    ...
    fn page_len(&self) -> usize {
        self.pages.get(self.page).map(|p| p.len()).unwrap_or(0)
    }
    ...
    pub fn update(&mut self, dt: f32) {
        ...
        if (self.revealed as usize) < len {
            ...
            self.wait_t += dt;
            if self.auto_advance && self.wait_t >= self.hold {
                self.advance();
            }
        }
    }
...
mod tests {
    ...
    fn parses_markup_and_pages() {
        let d = Dialog::new(
            "Hi {n}Link{/}, take the {i}Sword{/}.||Bye, {p}Hyrule{/}!",
            1000.0,
        );
    ...
    fn runs_carry_roles() {
        ...
        assert!(runs
            .iter()
            .any(|r| r.role == Role::Place && r.text.contains("home")));
    }
}
