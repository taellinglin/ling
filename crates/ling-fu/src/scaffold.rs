...
impl ProjectKind {
    pub fn from_str(s: &str) -> Self {
        match s {
            ...
            "lib" | "共修" | "라이브러리" | "ライブラリ" | "ไลบรารี" => ProjectKind::Lib,
            "game" | "游灵" | "게임" | "ゲーム" | "เกม" => ProjectKind::Game,
            "ui" | "显灵" | "UI" | "인터페이스" | "インターフェース" => ProjectKind::Ui,
            "ai" | "智灵" | "AI" | "인공지능" | "人工知能" | "ปัญญา" => ProjectKind::Ai,
            "crypto" | "密灵" | "암호" | "暗号" | "การเข้ารหัส" => ProjectKind::Crypto,
            "web" | "网灵" | "웹" | "ウェブ" | "เว็บ" => ProjectKind::Web,
            "polyglot" | "万言" | "다국어" | "多言語" | "หลายภาษา" => ProjectKind::Polyglot,
            _ => ProjectKind::Bin,
        }
    }
    ...
    pub fn to_chinese(&self) -> &'static str { self.name_in(&ScaffoldLang::Chinese) }
    pub fn to_english(&self) -> &'static str { self.name_in(&ScaffoldLang::English) }
}
...
fn write_manifest(dir: &Path, p: &ProjectScaffold, slang: &ScaffoldLang, dirs: &DirNames) -> anyhow::Result<()> {
...
# 添加依赖:  灵依名 = {{ 版本 = "x.y.z" }}
"#,
            name  = p.name,    kind  = p.kind.to_chinese(),
            lang  = p.language, level = p.spirit_level,
            src   = dirs.src,  lex   = dirs.lexicons,
...
"#,
            name  = p.name,    kind  = p.kind.name_in(slang),
            lang  = p.language, level = p.spirit_level,
            src   = dirs.src,  root  = p.root_language
        ),
...
"#,
            name  = p.name,    kind  = p.kind.to_english(),
            lang  = p.language, level = p.spirit_level,
            src   = dirs.src,  root  = p.root_language
        ),
    };
...
fn write_readme(dir: &Path, p: &ProjectScaffold, slang: &ScaffoldLang, dirs: &DirNames) -> anyhow::Result<()> {
    let (cli, build_cmd, run_cmd, test_cmd, src_dir, lex_dir, test_dir, build_dir) = match slang {
        ...
        ScaffoldLang::Thai     => ("ลิงฟู", "สร้าง", "เรียก", "ทดสอบ", "ต้นกำเนิด/", "คำศัพท์/", "ทดสอบ/", "สร้าง/"),
        _                      => ("lingfu", "build", "run",  "test", "src/",     "lexicons/", "tests/", "build/"),
    };
    let content = format!(
r#"# {name}
...
"#,
        name = p.name,    kind = p.kind.name_in(slang),
        level = p.spirit_level, lang = p.language,
        cli = cli,        build_cmd = build_cmd,
        run_cmd = run_cmd, test_cmd = test_cmd,
        manifest = dirs.manifest, src_dir = src_dir,
        entry = dirs.entry, lex_dir = lex_dir,
        test_dir = test_dir, build_dir = build_dir,
    );
...
fn write_entry(dir: &Path, p: &ProjectScaffold, slang: &ScaffoldLang, dirs: &DirNames) -> anyhow::Result<()> {
    let path = dir.join(dirs.src).join(dirs.entry);
    let content = match slang {
        ScaffoldLang::Chinese => format!(
"# {src}/{entry} — 灵境之门\n\n令 启 = 执 {{\n    印(\"🌊 欢迎来到 {name} 灵境 🌊\")\n    印(\"灵级: {level}\")\n\n    # 你的灵境代码从此觉醒\n}}\n",
            src = dirs.src, entry = dirs.entry,
            name = p.name, level = p.spirit_level),
        ScaffoldLang::Japanese => format!(
"# {src}/{entry} — 霊境への扉\n\n束縛 啓 = 実行 {{\n    印刷(\"🌊 ようこそ {name} 霊境へ 🌊\")\n    印刷(\"霊級: {level}\")\n\n    # あなたの霊境コードがここから目覚める\n}}\n",
            src = dirs.src, entry = dirs.entry,
            name = p.name, level = p.spirit_level),
...
fn write_lexicons(dir: &Path, slang: &ScaffoldLang, dirs: &DirNames) -> anyhow::Result<()> {
    ...
    fs::write(lex_dir.join("en.ling"), r#"name = "English"
code = "en"
canonical = true
...
"#)?;
    ...
    match slang {
        ScaffoldLang::Chinese => {
            fs::write(lex_dir.join("zh.ling"), r#"name = "中文"
code = "zh"
canonical = false
base = "en"
...
"#)?;
        ...
        ScaffoldLang::Japanese => {
            fs::write(lex_dir.join("ja.ling"), r#"name = "日本語"
code = "ja"
canonical = false
base = "en"
...
"#)?;
        ...
        ScaffoldLang::Korean => {
            fs::write(lex_dir.join("ko.ling"), r#"name = "한국어"
code = "ko"
canonical = false
base = "en"
...
"#)?;
        ...
        ScaffoldLang::Thai => {
            fs::write(lex_dir.join("th.ling"), r#"name = "ภาษาไทย"
code = "th"
canonical = false
base = "en"
...
fn write_test_file(dir: &Path, p: &ProjectScaffold, slang: &ScaffoldLang, dirs: &DirNames) -> anyhow::Result<()> {
    let path = dir.join(dirs.tests).join(dirs.test_file);
    let content = match slang {
        ScaffoldLang::Chinese => format!(
"# {tests}/{file} — 灵境之镜\n\n令 试 = 执 {{\n    # 验证基本算术 (Verify basic arithmetic)\n    若 1 + 1 == 2 {{\n        印(\"✨ 万镜皆明：算术正确 ✨\")\n    }}\n    印(\"🔮 试灵完成 for {name}\")\n}}\n",
            tests = dirs.tests, file = dirs.test_file, name = p.name),
        ScaffoldLang::Japanese => format!(
"# {tests}/{file} — 霊境の鏡\n\n束縛 試 = 実行 {{\n    もし 1 + 1 == 2 {{\n        印刷(\"✨ 全ての鏡が明らか ✨\")\n    }}\n    印刷(\"🔮 霊テスト完了: {name}\")\n}}\n",
            tests = dirs.tests, file = dirs.test_file, name = p.name),
        ScaffoldLang::Korean => format!(
...
fn write_lib_files(dir: &Path, p: &ProjectScaffold, slang: &ScaffoldLang, dirs: &DirNames) -> anyhow::Result<()> {
    let core_dir = dir.join(dirs.src).join(dirs.lib_core);
    let lib_file = core_dir.join(dirs.entry);
    let content = match slang {
        ScaffoldLang::Chinese => format!(
"# {src}/{core}/{entry} — 共修核心模块\n\n函 问好(名字) {{\n    归 格式(\"🌸 你好，{{}}！来自 {} 灵库\", 名字)\n}}\n",
            p.name, src = dirs.src, core = dirs.lib_core, entry = dirs.entry),
        ScaffoldLang::Japanese => format!(
...
fn write_game_files(dir: &Path, p: &ProjectScaffold, slang: &ScaffoldLang, dirs: &DirNames) -> anyhow::Result<()> {
    let content = match slang {
        ScaffoldLang::Chinese => format!(
...
fn write_ui_files(dir: &Path, p: &ProjectScaffold, slang: &ScaffoldLang, dirs: &DirNames) -> anyhow::Result<()> {
    let content = match slang {
        ScaffoldLang::Chinese => format!(
...
fn write_ai_files(dir: &Path, p: &ProjectScaffold, slang: &ScaffoldLang, dirs: &DirNames) -> anyhow::Result<()> {
    let content = match slang {
        ScaffoldLang::Chinese => format!(
"# AI 模型 — {name}\n\n令 启 = 执 {{\n    印(\"🧠 智灵模型 {name} 已就绪\")\n    # 训练 / 推理代码放在这里\n}}\n", name = p.name),
        _ => format!(
...
fn write_crypto_files(dir: &Path, p: &ProjectScaffold, slang: &ScaffoldLang, dirs: &DirNames) -> anyhow::Result<()> {
    let content = match slang {
        ScaffoldLang::Chinese => format!(
"# 密灵模块 — {name}\n\n令 启 = 执 {{\n    印(\"🔒 密灵 {name} 护法\")\n    # 加密 / 解密代码放在这里\n}}\n", name = p.name),
        _ => format!(
...
fn write_web_files(dir: &Path, p: &ProjectScaffold, slang: &ScaffoldLang, dirs: &DirNames) -> anyhow::Result<()> {
    let content = match slang {
        ScaffoldLang::Chinese => format!(
"# 网灵服务 — {name}\n\n令 启 = 执 {{\n    印(\"🌐 网灵 {name} 启航\")\n    # Web / WASM 代码放在这里\n}}\n", name = p.name),
        _ => format!(
...
"name = \"{name}\"\ncode = \"{code}\"\ncanonical = {canonical}\n\n[keywords]\nbind = \"{bind_kw}\"\nstart = \"{start_kw}\"\nprint = \"{print_kw}\"\nuse = \"{use_kw}\"\n",
            ...
            bind_kw = bind_kw, start_kw = start_kw,
            print_kw = print_kw, use_kw = use_kw,
        );
...
"#;
    fs::write(dir.join(dirs.src).join("shared").join("greetings.ling"), greet)?;
    Ok(())
}
