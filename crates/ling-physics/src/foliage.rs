...
impl Rng {
    ...
    fn range(&mut self, lo: f32, hi: f32) -> f32 { lo + self.next_f32() * (hi - lo) }
}
...
fn gen_branches(p: &TreeParams, start: Vec3, dir: Vec3, radius: f32, depth: u32, wind_w: f32, rng: &mut Rng) -> Vec<Branch> {
    if depth >= p.max_depth { return vec![]; }
    ...
    if depth + 1 < p.max_depth {
        for i in 0..p.branch_count {
            ...
            out.extend(gen_branches(p, end, child_dir, child_r, depth + 1, child_w, rng));
        }
    }
    out
}
...
fn spread_dir(axis: Vec3, azimuth: f32, elevation: f32) -> Vec3 {
    let right = if axis.abs().dot(Vec3::X) < 0.9 { axis.cross(Vec3::X) } else { axis.cross(Vec3::Z) };
...
fn cylinder_mesh(start: Vec3, end: Vec3, r: f32, sides: u32,
                 wind_w: f32, verts: &mut Vec<[f32; 3]>, norms: &mut Vec<[f32; 3]>,
                 idxs: &mut Vec<u32>, weights: &mut Vec<f32>) {
...
impl TreeParams {
    pub fn generate(&self) -> TreeMesh {
        ...
        let branches    = gen_branches(self, Vec3::ZERO, Vec3::Y, self.trunk_radius, 0, 0.0, &mut rng);
        let mut bv = Vec::new(); let mut bn = Vec::new(); let mut bi = Vec::new(); let mut bw = Vec::new();
        let mut lv = Vec::new(); let mut ln = Vec::new(); let mut li = Vec::new();
        let sides = 6u32;
        for b in &branches {
            cylinder_mesh(b.start, b.end, b.radius, sides, b.wind_w, &mut bv, &mut bn, &mut bi, &mut bw);
            if b.depth + 1 >= self.max_depth {
                ...
                let right = if n.abs().dot(Vec3::X) < 0.9 { n.cross(Vec3::X) } else { n.cross(Vec3::Z) };
                ...
                lv.push((b.end - right + up).to_array());
                for _ in 0..4 { ln.push(n.to_array()); }
                li.extend_from_slice(&[base, base+1, base+2, base, base+2, base+3]);
            }
        }
        TreeMesh { bark_verts: bv, bark_normals: bn, bark_indices: bi, wind_weights: bw,
                   leaf_verts: lv, leaf_normals: ln, leaf_indices: li }
    }
    ...
    pub fn wind_deform(&self, mesh: &mut TreeMesh, wind_dir: [f32; 2], strength: f32, time: f32) {
        for (i, v) in mesh.bark_verts.iter_mut().enumerate() {
            ...
            let wave = ((time * 2.1 + v[1] * 0.5).sin() * 0.6
                       + (time * 3.7 + v[1] * 0.9).sin() * 0.4) * strength * w;
            v[0] += wind_dir[0] * wave;
            v[2] += wind_dir[1] * wave;
        }
