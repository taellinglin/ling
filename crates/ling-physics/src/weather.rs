...
impl Default for WorldTime {
    fn default() -> Self { Self { hour: 8.0, day: 0, day_length: 120.0 } }
}
...
impl WorldTime {
    ...
    pub fn is_day(&self) -> bool { self.hour >= 6.0 && self.hour < 20.0 }
...
pub enum WeatherState { Clear, Cloudy, Overcast, Rain, Snow, Storm, Fog }
...
impl Atmosphere {
    ...
    pub fn moon_direction(&self) -> Vec3 { -self.sun_direction() }
    ...
    pub fn sky_color(&self) -> [f32; 3] {
        ...
        let (lo, hi) = stops.windows(2).find(|w| looped_hour >= w[0].0 && looped_hour < w[1].0)
            .map_or((stops[6], stops[0]), |w| (w[0], w[1]));
        let frac = ((looped_hour - lo.0) / (hi.0 - lo.0)).clamp(0.0, 1.0);
        let lerp = |a: f32, b: f32| a + (b - a) * frac;
        let base = [lerp(lo.1[0], hi.1[0]), lerp(lo.1[1], hi.1[1]), lerp(lo.1[2], hi.1[2])];
        // Darken by cloud cover.
        let cover = self.weather.cloud_cover;
        [base[0] * (1.0 - cover * 0.4), base[1] * (1.0 - cover * 0.35), base[2] * (1.0 - cover * 0.2)]
    }
    ...
    pub fn ambient_light(&self) -> [f32; 3] {
        ...
        [sky[0] * 0.35 * df + 0.01, sky[1] * 0.35 * df + 0.01, sky[2] * 0.35 * df + 0.02]
    }
    ...
    pub fn fog_color(&self) -> [f32; 3] {
        let sky = self.sky_color();
        [sky[0] * 0.9 + 0.05, sky[1] * 0.9 + 0.05, sky[2] * 0.9 + 0.05]
    }
    ...
    pub fn gust(&self, real_time: f32) -> f32 {
        ...
        (base + (real_time * 0.4).sin() * 0.1 + (real_time * 1.3).sin() * 0.05)
            .clamp(0.0, 1.0)
    }
