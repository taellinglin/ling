...
impl EaseFunction {
    ...
    pub fn from_name(name: &str) -> Self {
        let n: String = name.to_lowercase().chars()
            .filter(|c| c.is_ascii_alphanumeric()).collect();
        match n.as_str() {
            "linear" => Self::Linear,
            "step" => Self::Step,
            "quadin" => Self::QuadIn, "quadout" => Self::QuadOut, "quadinout" => Self::QuadInOut,
            "cubicin" => Self::CubicIn, "cubicout" => Self::CubicOut, "cubicinout" | "smooth" => Self::CubicInOut,
            "sinein" => Self::SineIn, "sineout" => Self::SineOut, "sineinout" => Self::SineInOut,
            "expoin" => Self::ExpoIn, "expoout" => Self::ExpoOut, "expoinout" => Self::ExpoInOut,
            "elasticin" => Self::ElasticIn, "elasticout" | "elastic" => Self::ElasticOut,
            "backin" => Self::BackIn, "backout" | "overshoot" => Self::BackOut, "backinout" => Self::BackInOut,
            "bouncein" => Self::BounceIn, "bounceout" | "bounce" => Self::BounceOut,
            _ => Self::Linear,
        }
    }
    ...
    pub fn apply(self, t: f32) -> f32 {
        ...
        match self {
            ...
            Self::QuadInOut => if t < 0.5 { 2.0*t*t } else { -1.0 + (4.0 - 2.0*t)*t },
            Self::CubicIn => t*t*t,
            Self::CubicOut => { let s = t - 1.0; s*s*s + 1.0 }
            Self::CubicInOut => if t < 0.5 { 4.0*t*t*t } else { (t-1.0)*(2.0*t-2.0)*(2.0*t-2.0) + 1.0 },
            Self::SineIn => 1.0 - (t * FRAC_PI_2).cos(),
            Self::SineOut => (t * FRAC_PI_2).sin(),
            Self::SineInOut => 0.5 * (1.0 - (PI * t).cos()),
            Self::ExpoIn => if t == 0.0 { 0.0 } else { 2.0_f32.powf(10.0*t - 10.0) },
            Self::ExpoOut => if t == 1.0 { 1.0 } else { 1.0 - 2.0_f32.powf(-10.0*t) },
            Self::ExpoInOut => {
            ...
            Self::BackIn => { let c1 = 1.70158; let c3 = c1 + 1.0; c3*t*t*t - c1*t*t }
            Self::BackOut => { let c1 = 1.70158; let c3 = c1 + 1.0; 1.0 + c3*(t-1.0).powi(3) + c1*(t-1.0).powi(2) }
            Self::BackInOut => {
...
mod tests {
    ...
    fn endpoints_are_fixed() {
        for f in [EaseFunction::CubicInOut, EaseFunction::BounceOut, EaseFunction::ElasticOut,
                  EaseFunction::BackInOut, EaseFunction::ExpoInOut, EaseFunction::SineInOut] {
    ...
    fn from_name_aliases() {
        assert_eq!(EaseFunction::from_name("cubic-in-out"), EaseFunction::CubicInOut);
