...
pub enum Region {
    Cranium, Skull, Scalp, Brow, Eye, Lid, Iris, Nose, Cheek, Mouth, Jaw,
}
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, serde::Serialize, serde::Deserialize)]
pub enum Expression {
    Blink, Smile, Frown, Surprise, Squint, JawOpen, Anger, Sad,
}
...
impl FaceRig {
    pub fn new() -> Self { Self { points: topology() } }
    ...
    pub fn solve_point(&self, name: &str, weights: &[(Expression, f32)]) -> Option<Vec2> {
        ...
        let rigid = self.points.iter().find(|p| p.name == name).map(|p| p.rigid).unwrap_or(false);
        if rigid { return Some(base); }
        let mut off = Vec2::ZERO;
        for &(e, w) in weights { off += e.offset(name) * w; }
        Some(base + off)
    }
    ...
    pub fn solve(&self, weights: &[(Expression, f32)]) -> Vec<Vec2> {
        self.points.iter().map(|p| {
            if p.rigid { return p.rest; }
            let mut off = Vec2::ZERO;
            for &(e, w) in weights { off += e.offset(p.name) * w; }
            p.rest + off
        }).collect()
    }
    ...
    pub fn eye_aperture_l(&self, weights: &[(Expression, f32)]) -> f32 {
        let u = self.solve_point("lid_upper_l", weights).unwrap_or(Vec2::ZERO);
        let l = self.solve_point("lid_lower_l", weights).unwrap_or(Vec2::ZERO);
        (u.y - l.y).max(0.0)
    }
}
...
mod tests {
    ...
    fn topology_is_symmetric_and_complete() {
        let t = topology();
        assert!(t.len() >= 40, "expected a rich landmark set, got {}", t.len());
        // every "_l" landmark has a mirrored "_r" with negated x
        for fp in &t {
            if let Some(stem) = fp.name.strip_suffix("_l") {
                let r = t.iter().find(|q| q.name == format!("{stem}_r")).expect("mirror");
                assert!((fp.rest.x + r.rest.x).abs() < 1e-4, "{} not mirrored", fp.name);
                assert!((fp.rest.y - r.rest.y).abs() < 1e-4, "{} y mismatch", fp.name);
            }
        }
    }
    ...
    fn blink_closes_the_eye() {
        ...
        assert!(shut < open * 0.25, "blink should mostly close: {open} -> {shut}");
    }
    ...
    fn smile_lifts_mouth_corners() {
        ...
        let smiled = rig.solve_point("mouth_corner_l", &[(Expression::Smile, 1.0)]).unwrap();
    ...
    fn rigid_skull_points_never_move() {
        ...
        let moved = rig.solve_point("zygomatic_l", &[(Expression::Smile, 1.0), (Expression::JawOpen, 1.0)]).unwrap();
    ...
    fn weights_blend_linearly() {
        let rig = FaceRig::new();
        let full = rig.solve_point("mouth_corner_l", &[(Expression::Smile, 1.0)]).unwrap();
        let half = rig.solve_point("mouth_corner_l", &[(Expression::Smile, 0.5)]).unwrap();
        let rest = rig.rest("mouth_corner_l").unwrap();
