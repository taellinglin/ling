...
pub fn detect_language(source: &str) -> &'static str {
    ...
    let max   = counts.iter().enumerate().max_by_key(|&(_, v)| v).map(|(i, _)| i).unwrap_or(0);
    langs[max]
}
...
mod tests {
    use super::*;
    #[test] fn detects_thai()   { assert_eq!(detect_language("ฟังก์ชัน หลัก"), "th"); }
    #[test] fn detects_chinese(){ assert_eq!(detect_language("令 x = 若"), "zh"); }
    #[test] fn detects_english(){ assert_eq!(detect_language("fn main() {}"), "en"); }
}
