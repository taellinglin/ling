...
pub enum Expr {
    ...
    // Binding / assignment
    Bind  { name: String, ty: Option<TypeExpr>, value: Box<Expr>, span: Span },
    Assign { target: Box<Expr>, value: Box<Expr>, span: Span },

    // Control flow
    If    { cond: Box<Expr>, then: Box<Expr>, else_: Option<Box<Expr>>, span: Span },
...
impl Expr {
    pub fn span(&self) -> Span {
        match self {
            Self::Number(_) | Self::Str(_) | Self::Bool(_) | Self::Unit | Self::Nil
            | Self::Ident(_) => Span::DUMMY,
            Self::Block(_, s) | Self::Call { span: s, .. } | Self::Index { span: s, .. }
            | Self::Field { span: s, .. } | Self::Bind { span: s, .. }
            | Self::Assign { span: s, .. } | Self::If { span: s, .. }
            | Self::While { span: s, .. } | Self::For { span: s, .. }
            | Self::Return(_, s) | Self::Break(s) | Self::Continue(s)
            | Self::BinOp { span: s, .. } | Self::UnOp { span: s, .. }
            | Self::Lambda { span: s, .. } | Self::List(_, s)
            | Self::Tuple(_, s) | Self::Map(_, s) => *s,
        }
    }
}
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, Serialize, Deserialize)]
pub enum UnOp { Neg, Not, Ref, Deref }
