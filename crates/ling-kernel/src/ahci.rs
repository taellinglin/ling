...
fn stop_command_engine() {
    ...
    while port_reg_read(PX_CMD) & (CMD_FR | CMD_CR) != 0 {
        unsafe { crate::cpu::pause() };
    }
}

fn start_command_engine() {
    while port_reg_read(PX_CMD) & CMD_CR != 0 {
        unsafe { crate::cpu::pause() };
    }
    let mut cmd = port_reg_read(PX_CMD);
    cmd |= CMD_FRE;
...
fn issue(lba: u32, buf_ptr: *mut u8, write: bool) -> Result<(), ()> {
    ...
    while port_reg_read(PX_TFD) & 0x88 != 0 {
        unsafe { crate::cpu::pause() };
    }
    ...
    loop {
        if port_reg_read(PX_CI) & 1 == 0 {
            break;
        }
        if port_reg_read(PX_IS) & (1 << 30) != 0 {
            return Err(()); // TFES: task file error status
        }
        unsafe { crate::cpu::pause() };
    }
    Ok(())
